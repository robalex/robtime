...
function PositionsTab({ employeeId, employeeIdParam }: { employeeId: number; employeeIdParam: string }) {
  const { data: assignments, isPending, isError, error } = usePositionAssignments(employeeId)
  ...
  if (isError) {
    return (
      <p className="text-sm text-destructive">
        {toApiProblem(error, 'Could not load position assignments.').message}
      </p>
    )
  }
  ...
  const periods: TimelinePeriod[] = assignments.map((assignment) => ({
    ...
    label: `${assignment.positionCode} — ${assignment.positionName}`,
    sublabel:
      assignment.rate != null
        ? `$${assignment.rate.toFixed(2)}/hr (override)`
        : `$${assignment.positionBaseRate.toFixed(2)}/hr (position rate)`,
  }))

  return (
    <EffectiveDatedTimeline
      periods={periods}
      emptyMessage="No position assigned yet. Assign one to start paying this employee."
      changeEffectiveAction={
        <Link
          to="/people/$employeeId/positions/new"
