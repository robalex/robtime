...
public class EndToEndTests
{
    ...
    private static Punch In(Employee emp, Instant t, int? positionId = null) =>
        TestEntityCreator.CreateTestPunch(t, PunchKind.In, emp) with { PositionId = positionId };
    private static Punch Out(Employee emp, Instant t, int? positionId = null) =>
        TestEntityCreator.CreateTestPunch(t, PunchKind.Out, emp) with { PositionId = positionId };
    ...
    public void OvernightAndHolidayDifferentials_StackAndFeedRegularRate_EndToEnd()
    {
        ...
        Assert.Contains(result.LineItems, l => l.Type == PayLineType.Differential && l.Description.Contains("HOLIDAY") && l.Amount == 10m);
        Assert.Contains(result.LineItems, l => l.Type == PayLineType.Differential && l.Description.Contains("NIGHT") && l.Amount == 12m);
    }
    ...
    public void ExclusivityGroup_OnlyHighestDifferentialApplies_EndToEnd()
    {
        ...
        Assert.Contains(result.LineItems, l => l.Description.Contains("HIGH"));
        Assert.DoesNotContain(result.LineItems, l => l.Description.Contains("LOW"));
    }
